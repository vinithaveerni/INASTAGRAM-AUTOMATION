	   ...
	   // navigate to instagram page  
		...
		//  login credentials //----------------------------------------------------------------
	     ...
	     // login button
	        ...
	        List<String> accounts = Arrays.asList("account 1","account 2","account 3");

	        for (String accountfollows : accounts) {
	          work.moveToElement(driver.findElement(By.xpath("//*[@aria-label='Home']"))).click().perform();
	          Thread.sleep(3000);
	        
	        //Search for the Desired Account
	        work.moveToElement(driver.findElement(By.xpath("//a[@href='#']"))).click().sendKeys(accountfollows).perform();
          Thread.sleep(5000);
          ...
          for (WebElement result : list) {
              ...
              if (result.getText().contains(accountfollows)) {
                  work.click(result).perform();
                  Thread.sleep(3000);
                 
          // Follow the Account  
          WebElement followButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[.='Follow' or .='Following' or  .='Requested']")));
          
          String buttonText = followButton.getText(); 
          if (buttonText.equals("Follow")) {
            work.click(followButton).perform();
            System.out.println("Follow request sent to :" + accountfollows);
          }
          else {
              
              System.out.println("Already following or follow request already sent to :" + accountfollows);
              } 
          try {
           ...
           break; // Break the inner loop once the correct account is found and followed
